...
inline Color diffuseComponent(const Light *i_light, HitInfo i_hInfo, Color i_diffuse)
{
	Color o_diffuseComponent = Color(0, 0, 0);
	float geometryTerm = ((-1)*i_light->Direction(i_hInfo.p)).Dot(i_hInfo.N);
	if (geometryTerm > 0)
		o_diffuseComponent = i_light->Illuminate(i_hInfo.p, i_hInfo.N) * i_diffuse * geometryTerm;

	o_diffuseComponent.ClampMinMax();
	return o_diffuseComponent;
}

inline Color ambientComponent(const Light *i_light, HitInfo i_hInfo, Color i_diffuse)
{
	Color o_ambient = Color(0, 0, 0);
	o_ambient = i_light->Illuminate(i_hInfo.p, i_hInfo.N) * i_diffuse;
